...
inline tid_t StructurePersistence::create_struct_with_substructs(
    SynthStruct& synth_struct,
    qvector<SubStructInfo>& sub_structs)
{
...
inline tinfo_t StructurePersistence::create_bitfield_base_type(uint32_t size) {
    tinfo_t type;
...
inline SemanticType StructurePersistence::semantic_from_type(const tinfo_t& type) {
    if (type.empty()) {
...
inline StructurePersistence::StructSignature StructurePersistence::build_signature(
    const SynthStruct& synth_struct)
{
    StructSignature sig;
...
inline bool StructurePersistence::build_signature_from_tinfo(
    const tinfo_t& tif,
    StructSignature& out)
{
    out.fields.clear();
...
inline double StructurePersistence::compute_similarity(
...
inline std::optional<std::tuple<tid_t, qstring, double>> StructurePersistence::find_reuse_candidate(
    const SynthStruct& synth_struct,
    double threshold)
{
